...
void SP2_WinScene::Init()
{
	...
	meshList[GEO_WINSCREEN] = MeshBuilder::GenerateQuad("Win Screen", Color(0, 0, 0), 1.f, 1.f);
...
void SP2_WinScene::Update(double dt)
{
	FPS = 1.f / (float)dt;

	if (Application::IsKeyPressed(VK_RETURN))
	{
		Application::SceneSetter = 20;
	}
...
string SP2_WinScene::UpdateFrameRate(float fps)
{
...
void SP2_WinScene::RenderMesh(Mesh *mesh, bool enableLight) //Pass in mesh name to go through this to enable lighting for that mesh
{
...
void SP2_WinScene::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SP2_WinScene::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
	glDisable(GL_DEPTH_TEST);
