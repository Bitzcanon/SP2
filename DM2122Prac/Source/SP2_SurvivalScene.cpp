...
SP2_SurvivalScene::SP2_SurvivalScene()
{
...
void SP2_SurvivalScene::UpdateBuffs(double dt)
{
	//if (SceneCar.getXpos() )
}

void SP2_SurvivalScene::Update(double dt)
{
	FPS = 1.f / (float)dt;
	g_dElapsedTime += dt;
	g_dDeltaTime = dt;
...
string SP2_SurvivalScene::UpdateFrameRate(float fps)
{
...
void SP2_SurvivalScene::RenderMesh(Mesh *mesh, bool enableLight) //Pass in mesh name to go through this to enable lighting for that mesh
{
...
void SP2_SurvivalScene::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SP2_SurvivalScene::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
	glDisable(GL_DEPTH_TEST);
...
void SP2_SurvivalScene::RenderSkybox()
{
	//Left side of Skybox
	modelStack.PushMatrix();
	{
...
void SP2_SurvivalScene::Render()
{
	...
	modelStack.PushMatrix();
	{
		...
		modelStack.PushMatrix();
		{
			...
			RenderTextOnScreen(meshList[GEO_TEXT], to_string(timer), Color(1, 1, 0), 1, 70, 53);


			RenderTextOnScreen(meshList[GEO_TEXT], UpdateFrameRate(FPS), Color(1, 1, 0), 2, 73, 55);
